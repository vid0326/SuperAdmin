...
export default function AddUser() {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        ...
        setApiError("Could not load roles for selection.");
      }
    };
    fetchRoles();
  }, []);
  ...
  const validateField = (name, value) => {
    const fieldSchema = Joi.object({ [name]: userSchema.extract(name) });
    ...
    <div className="p-6">
      ...
      <div className="max-w-2xl mx-auto bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl">
        <div className="text-center mb-8">
          <UserPlus className="mx-auto h-10 w-10 text-indigo-400" />
          <h1 className="text-2xl font-bold mt-4 text-gray-800 dark:text-white">Create a New User</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">Fill out the details to add a new user.</p>
        </div>
